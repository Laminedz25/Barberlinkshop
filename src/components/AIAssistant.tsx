...
const AIAssistant = () => {
    ...
    const getMockResponse = (query: string): string => {
        ...
        if (q.includes("استثمار") || q.includes("invest") || q.includes("مستثمر") || q.includes("مشكلة") || q.includes("مساعدة متقدمة") || containsEmail) {
            return "عزيزي المستخدم، بصفتي وكيل الرد السريع، لقد قمت برفع استفسارك. يرجى ترك بريدك الإلكتروني إذا لم تفعل ذلك. سيقوم 'وكيل البريد الإلكتروني (Email Agent)' المخصص للردود المعقدة والاستثمارات بالتواصل معك قريباً وتزويدك بكافة التفاصيل بطريقة احترافية.";
        }
        ...
        if (userRole === 'mobile_barber') {
            return "أهلاً بك كحلاق متنقل (Mobile Barber)! أنا وكيلك الذكي. يمكنني مساعدتك في توضيح كيفية تفعيل خدمة الزيارات المنزلية (Home Visits) وإدارة الجدولة الجغرافية لزياراتك للزبائن.";
        }

        if (userRole === 'salon_barber') {
            ...
            return "أهلاً بك كحلاق داخل صالون! يمكنني مساعدتك في توضيح كيفية التواصل مع زبائنك لضمان تقديم تجربة ممتازة وتتبع عمولاتك.";
        }
